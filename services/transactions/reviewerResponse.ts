...
export async function updateReviewerResponseToAssignedTask(
  ...
  const tracsaction = await prisma.$transaction(async (db) => {
    ...
    const currentArticleStatus = await db.intoArticle.findUnique({
      where: {
    ...
    if (
      ...
      currentArticleStatus?.article_status ===
      ...
      if (isUpdated) {
        ...
        const acceptedCount: number = prevStatus.filter(
          (r) => r.is_accepted === "accepted" && r.round === Number(round)
        ).length;
        ...
        if (acceptedCount < 3) {
          ...
          await db.intoArticle.update({
            where: { intro_id: Number(article_id) },
            data: {
              article_status: ArticleStatus.submissionneedadditionalreviewers,
            },
          });
        }
